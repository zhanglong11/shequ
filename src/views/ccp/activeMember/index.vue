    ...
    <div class="main-content">
      <div class="tooltips">
        <el-form inline>
          ...
          <el-form-item>
            <Select v-model="filterForm.status" has-all arg-group="ccpActiveMemberStatus" placeholder="状态"></Select>
          </el-form-item>
          <el-form-item>
            <el-button-group>
          ...
          <el-form-item>
            <button-link
              v-if="$hasPower('CcpActiveMemberCreate')"
              to="activeMember/new"
              type="primary"
              plain
              icon="el-icon-plus"
      ...
      <Grid ref="table" page-use-query :request="request" :filter-form.sync="filterForm">
        ...
        <vxe-table-column title="操作" width="180px">
          <template #default="{ row }">
            <button-link v-if="$hasPower('CcpActiveMemberUpdate')" type="text" :to="'activeMember/' + row.id"
              >编辑</button-link
            >
            <button-link
              v-if="$hasPower('CcpActiveMemberDetail')"
              type="text"
              :to="'activeMember/' + row.id + '/detail?showDelete=true'"
              >查看详情</button-link
            >
            <el-button v-if="$hasPower('CcpActiveMemberRemove')" type="text" @click="handleRemove(row)">删除</el-button>
          </template>
        </vxe-table-column>
      </Grid>
    </div>
...
import { getPartyActivistManagementList, removePartyActivistManagementById } from '@/api/ccp/activeMember'
...
export default {
  ...
  data() {
    return {
      activeId: null,
      filterForm: {},
      request: getPartyActivistManagementList
    }
  },
  created() {},
  mounted() {},
  methods: {
    ...
    async handleRemove(row) {
      ...
      await removePartyActivistManagementById(row.id)
