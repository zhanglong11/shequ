  ...
  <div class="container">
    <div class="tooltips">
      <el-form inline :model="filterForm">
        ...
        <el-form-item>
          <el-button
            v-if="$hasPower('MeetingManageAdd')"
            type="primary"
            plain
            icon="el-icon-plus"
    ...
    <Grid ref="table" :request="request" :filter-form.sync="filterForm">
      ...
      <vxe-table-column title="参会人员" show-overflow field="participantName"> </vxe-table-column>
      ...
      <vxe-table-column field="status" title="会议状态">
        <template #default="{ row }">
          <MeetingStatus v-model="row.status" />
        </template>
      </vxe-table-column>
      ...
      <vxe-table-column title="操作" width="260px" fixed="right">
        <template #default="{ row }">
          ...
          <el-button v-if="$hasPower('MeetingManageDetail')" type="text" @click="handleDetail(row)">查看详情</el-button>
        </template>
      </vxe-table-column>
    </Grid>
    <!-- 新建,编辑，取消模态框 -->
...
import MeetingStatus from './components/meeting'
import Api from '../api'
export default {
  name: 'MeetingManage',
  components: { Edit, CompanyDetail, MeetingStatus },
  data() {
    return {
      statusList,
