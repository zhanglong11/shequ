  ...
  <div class="container">
    <div class="tooltips">
      <el-form inline>
        ...
        <el-form-item>
          <el-button
            v-if="$hasPower('VoluntaryActivityAdd')"
            type="primary"
            plain
            icon="el-icon-plus"
            ...
            >新建活动</el-button
          >
          <el-button v-if="$hasPower('VolunteerActivityCode')" icon="el-icon-time" @click="handleCode"
            >二维码有效期</el-button
          >
        </el-form-item>
      </el-form>
    ...
    <Grid ref="table" :request="request" :filter-form.sync="filterForm">
      ...
      <vxe-table-column title="操作" width="350px" fixed="right">
        <template #default="{ row }">
          <el-button v-if="$hasPower('VoluntaryActivityEdit')" type="primary" @click="handleAdd(row.id)"
            >编辑</el-button
          >
          <el-button v-if="$hasPower('VoluntaryActivitySian')" type="primary" @click="handlePick(row.id)"
            >发起签到</el-button
          >
          <el-button v-if="$hasPower('VoluntaryActivityUpload')" type="primary" @click="handleCount(row.id)"
            >上传活动总结</el-button
          >
          <el-button v-if="$hasPower('VoluntaryActivityDetail')" type="primary" @click="handleDetail(row.id)"
            >查看详情</el-button
          >
          <el-button v-if="$hasPower('VoluntaryActivityDelete')" type="danger" @click="handleRemove(row.id)"
            >删除</el-button
          >
        </template>
      </vxe-table-column>
    ...
    <!--新建 编辑-->
    <VolAdd v-if="editDialogVisible" :id="voluntaryId" :visible.sync="editDialogVisible" />
    <!--上传签到-->
    <SignAdd v-if="signDialogVisible" :id="signId" :visible.sync="signDialogVisible" />
    <!--上传总结-->
    <SumAdd v-if="sumDialogVisible" :id="sumId" :visible.sync="sumDialogVisible" />

    <MeetingDetail v-if="detailDialogVisible" :id="detailId" :visible.sync="detailDialogVisible" />

    <!-- 二维码有效期模态框组件 start -->
    <ModalPeriod :visible.sync="periodVisible" code="volunteerActivity" />
    <!-- 二维码有效期模态框组件 end -->
  </div>
</template>
...
import SignAdd from './sign'
import SumAdd from './sum'
import MeetingDetail from '../components/detail'
import Api from '../api'
import { statusList } from '../lib/optionsList'
import ModalPeriod from '../components/ModalPeriod'
export default {
  name: 'VoluntaryActivity',
  components: { VolAdd, SignAdd, SumAdd, MeetingDetail, ModalPeriod },
  data() {
    return {
      statusList,
      activityList: [],
