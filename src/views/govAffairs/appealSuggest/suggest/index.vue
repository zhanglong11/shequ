  ...
  <div class="container">
    <div class="no-footer">
      <div class="tooltips">
        <el-form inline>
          <el-form-item>
            <el-input v-model="filterForm.applicant" placeholder="意见人"></el-input>
          </el-form-item>
      ...
      >
        <vxe-table-column title="意见人" field="applicant" width="200"></vxe-table-column>
        <vxe-table-column title="意见内容" field="content" min-width="200"></vxe-table-column>
        ...
        <vxe-table-column title="操作" field="opts" width="200" fixed="right">
          <template #default="{ row }">
            <el-button
              v-if="$hasPower('AppealSuggestSuggestAppeal') && row.status === 0"
              type="primary"
              @click="handleHandleSuggest(row)"
              >处理</el-button
            >
            <el-button v-if="$hasPower('AppealSuggestSuggestDetail')" type="primary" @click="handleDetail(row)"
              >查看详情</el-button
            >
            <el-button v-if="$hasPower('AppealSuggestSuggestDelete')" type="danger" @click="handleDelete(row)"
              >删除</el-button
            >
          </template>
        </vxe-table-column>
    ...
    <DetailDialog v-if="detailVisible" :id="selectRowId" type="suggest" :visible.sync="detailVisible"></DetailDialog>
    <!--处理弹窗-->
    <HandleDialog
      v-if="handleVisible"
...
export default {
  ...
  methods: {
    ...
    async submitHandle(data) {
      await Api.submitHandleSuggest({ id: this.selectRow.id, ...data })
    ...
    async handleDelete(row) {
      ...
      if (res) {
        await Api.deleteSuggest(row.id)
