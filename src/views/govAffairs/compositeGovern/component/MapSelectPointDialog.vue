<template>
  <el-dialog title="选择地点" width="1400px" :visible="visible" @close="close">
    <div class="body">
...
export default {
  ...
  watch: {
    location: {
      ...
      handler(val) {
        if (val) {
          this.addPropMarker(val.split(',')[0], val.split(',')[1])
        }
      }
    }
  },
  ...
  methods: {
    ...
    async mapClick(e) {
      ...
      this.geocode.getLocation(e.lnglat, res => {
        this.$emit('update:location', `${e.lnglat.lng},${e.lnglat.lat}`)
          ...
          ;(function (i) {
            ...
            marker.addEventListener('click', function () {
              ...
              that.geocode.getLocation(lnglat, res => {
                that.$emit('update:location', `${lnglatArr[0]},${lnglatArr[1]}`)
                const pl = res.result.result.addressComponent.province
                const cl = res.result.result.addressComponent.city
                const al = res.result.result.addressComponent.county
    ...
    showPosition(item) {
      ...
      this.geocode.getLocation(lnglat, res => {
        this.$emit('update:location', `${lnglat.lng},${lnglat.lat}`)
        const pl = res.result.result.addressComponent.province
        const cl = res.result.result.addressComponent.city
        const al = res.result.result.addressComponent.county
        this.$emit('update:address', pl + cl + al + res.result.result.addressComponent.address)
